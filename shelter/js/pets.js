...
let maxVisibleItemsPets;
if (window.innerWidth >= 1280) {
    maxVisibleItemsPets = 8;
}
else if (window.innerWidth >= 768) {
    maxVisibleItemsPets = 6;
}
else if (window.innerWidth >= 450) {
    maxVisibleItemsPets = 3;
}

const cardsContainer = document.querySelector('.our-friends-container');

function createCards() {
    let copyCardsData = Array.from(cardsData);
    for (let i = 0; i < maxVisibleItemsPets; i++) {
...
const prevButton = document.querySelector('.pagination-arrows.nav-left');
const nextButton = document.querySelector('.pagination-arrows.nav-right');
let paginationNumber = document.querySelector('.page-number');
let pagCounter = 1;
let carouselItems = document.querySelectorAll('.friends-item');
const ourFriends = document.querySelector('.our-friends');
const distanceFromTop = ourFriends.offsetTop;

let safeElemRight = [];
let safeElemLeft = [];

paginationNumber.innerHTML = pagCounter;

function swipeSlider() {
    nextButton.addEventListener('click', () => {
        pagCounter++;
        paginationNumber.innerHTML = pagCounter;
        current = document.querySelectorAll('.friends-item');
        ...
        carouselItems = document.querySelectorAll('.friends-item');
        modalWindow();
...
function modalWindow() {
            ...
            cardsData.forEach((cardItem) => {
                if (cardItem.name == petName) {
                    ...
                    modalButton.addEventListener("click", function (e) {
                        cardsContainer.removeChild(modal);
    ...
    overlayModal.addEventListener("click", function (e) {
        ...
        if (modal && modal.parentNode === cardsContainer) {
            cardsContainer.removeChild(modal);
        }
    });
